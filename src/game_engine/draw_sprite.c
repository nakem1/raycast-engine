...
/*   Updated: 2021/02/08 22:22:46 by lmurray          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		calculate_limits_sprite(t_cub3d *cub)
{
	...
	cub->sprite.bottom_spr_y = cub->sprite.height_spr / 2 + \
			cub->window.res_height / 2;
...
void		transpons_matrix(t_cub3d *cub, int i)
{
	...
	cub->sprite.transpons_matrix_x = inverse * (cub->player.y_dir * \
			cub->sprite.x_spr - cub->player.x_dir * cub->sprite.y_spr);
	cub->sprite.transpons_matrix_y = inverse * ((cub->camera.y_plane * -1) * \
			cub->sprite.x_spr + cub->camera.x_plane * cub->sprite.y_spr);
	cub->sprite.spr_scrx = (int)((cub->window.res_width / 2) * (1 + \
			cub->sprite.transpons_matrix_x / cub->sprite.transpons_matrix_y));
}
...
void		sort_sprites(t_cub3d *cub)
{
	...
	while (i < cub->sprite.count_sprite)
	{
		cub->sprite.spr[i].dist = ((cub->player.x_pos - \
				cub->sprite.spr[i].spr_x) * (cub->player.x_pos - \
				cub->sprite.spr[i].spr_x) + (cub->player.y_pos - \
				cub->sprite.spr[i].spr_y) * (cub->player.y_pos - \
				cub->sprite.spr[i].spr_y));
		i++;
	}
	ft_sort(cub);
}
